--Begin Tools.lua :)
local SUDO = 460848425 -- put Your ID here! <===
...
if not matches[2] and msg.reply_id then
    tdcli_function ({
      ...
      chat_id_ = msg.to.id,
      message_id_ = msg.reply_id
    }, action_by_reply, {chat_id=msg.to.id,cmd="addadmin"})
  end
  if matches[2] and string.match(matches[2], '^%d+$') then
tdcli_function ({
    ID = "GetUser",
    user_id_ = matches[2],
  }, action_by_id, {chat_id=msg.to.id,user_id=matches[2],cmd="addadmin"})
    end
  if matches[2] and not string.match(matches[2], '^%d+$') then
   tdcli_function ({
      ID = "SearchPublicChat",
      username_ = matches[2]
    }, action_by_username, {chat_id=msg.to.id,username=matches[2],cmd="addadmin"})
      end
   end
...
if matches[1] == "helps" and is_mod(msg) then
if not lang then
text = [[
